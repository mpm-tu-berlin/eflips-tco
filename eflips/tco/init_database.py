...
def init_database(session, scenario):
    ...
    for vtype in all_vtypes:
        if vtype.name in p.vehicle_dict():
            tco_parameters = {
                "name": vtype.name,
                "procurement_cost": p.vehicle_dict()
                .get(vtype.name, {})
                .get("procurement_cost"),
                "useful_life": p.vehicle_dict().get(vtype.name, {}).get("useful_life"),
                "cost_escalation": p.vehicle_dict()
                .get(vtype.name, {})
                .get("cost_escalation"),
            }
        else:
            raise ValueError(
                "Please check your input data and add the missing parameters for this vehicle: "
                + vtype.name
        ...
        if vtype.name in p.battery_dict():
            tco_parameters_battery = {
                "name": vtype.name,
                "procurement_cost": p.battery_dict()
                .get(vtype.name, {})
                .get("procurement_cost"),
                "useful_life": p.battery_dict().get(vtype.name, {}).get("useful_life"),
                "cost_escalation": p.battery_dict()
                .get(vtype.name, {})
                .get("cost_escalation"),
            }
        else:
            raise ValueError(
                "Please check your input data and add the missing parameters for the battery of vehicle: "
                + vtype.name
                + ". \n Please also make sure that the spelling of the vehicle type in your input "
                "file is correct."
            )
        ...
        if vtype.battery_type_id is not None:
            battery_type = (
                ...
                .filter(BatteryType.id == vtype.battery_type_id)
                .first()
            )
        ...
        # if there is no battery type, it needs to be added.
            ...
            new_battery_type = BatteryType(
                scenario_id=vtype.scenario_id,
                specific_mass=1,
                chemistry="xyz",
                tco_parameters=tco_parameters_battery,
            )
    ...
    for cp in Charging_Points:
        if cp.name == "120 kw_Slot" and cp.scenario_id == scenario.id:
            Slot_120 = False
        if cp.name == "300 kw_Slot" and cp.scenario_id == scenario.id:
            Slot_300 = False
    if Slot_120:
        new_charge_point_120 = ChargingPointType(
            scenario_id=scenario.id, name="120 kw_Slot", name_short="120"
        )
        session.add(new_charge_point_120)
    if Slot_300:
        new_charge_point_300 = ChargingPointType(
            scenario_id=scenario.id, name="300 kw_Slot", name_short="300"
        )
    ...
    for chp in charging_points:
        if chp.name in p.charging_stations_dict():
            tco_parameters_charging_point = {
                "name": chp.name,
                "procurement_cost": p.charging_stations_dict()
                .get(chp.name, {})
                .get("procurement_cost"),
                "useful_life": p.charging_stations_dict()
                .get(chp.name, {})
                .get("useful_life"),
                "cost_escalation": p.charging_stations_dict()
                .get(chp.name, {})
                .get("cost_escalation"),
            }
        else:
            raise ValueError(
                "Please check your input data and add the missing parameters for the charging slot: "
                + chp.name
    ...
    all_electrified_areas = (
        session.query(Area)
        .join(AssocAreaProcess, AssocAreaProcess.area_id == Area.id)
        .join(Process, AssocAreaProcess.process_id == Process.id)
        .filter(Area.scenario_id == scenario.id, Process.electric_power is not None)
        .all()
    )
    ...
    dc_stations = (
        ...
        .filter(Station.is_electrified == True, Station.scenario_id == scenario.id)
        .all()
    )
    ...
    oc_stations = (
        session.query(Station)
        .outerjoin(Depot, Station.id == Depot.station_id)
